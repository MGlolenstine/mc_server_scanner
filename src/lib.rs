...
pub const LOCAL_IP: Ipv4Addr = Ipv4Addr::new(127, 0, 0, 1);
pub const LOCAL_IP_V6: Ipv6Addr = Ipv6Addr::new(0u16, 0u16, 0u16, 0u16, 0u16, 0u16, 0u16, 0u16);

/// Scan ports in Minecraft range, to find the Minecraft one.
pub async fn scan_ports() -> Vec<(ServerQueryResponse, u16)> {
    let all_ports = get_used_ports();
    let mut ports: Vec<(ServerQueryResponse, u16)> = vec![];
    for (addr, port) in all_ports {
        if ports.iter().all(|a| a.1 != port) {
...
pub async fn is_minecraft(addr: IpAddr, port: u16) -> Result<ServerQueryResponse> {
    ...
    let buf_len = tokio::time::timeout(Duration::from_millis(50), stream.read(&mut resp)).await;
    if buf_len.is_err() {
        debug!("Timed out!");
        bail!("Timed out!");
